...
export default function ProfileScreen() {
  ...
  const selectedEstate = useMemo(
    () => ESTATE_OPTIONS.find((option) => option.value === estateValue),
    [estateValue]
  );
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        <Text style={styles.label}>{t(language, 'labelName')}</Text>
        <TextInput style={styles.input} value={name} onChangeText={setName} />

        <Text style={styles.label}>Region</Text>
        <Pressable style={styles.selector} onPress={() => setPickerMode('region')}>
          <Text style={styles.selectorLabel}>{districtLabel(language, regionKey)}</Text>
        </Pressable>
        ...
        <View style={styles.rowWrap}>
          ...
          <Pressable
            style={[styles.chip, language === 'zh-Hant' && styles.chipActive]}
